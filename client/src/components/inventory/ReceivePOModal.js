...
const ReceivePOModal = ({ order, onClose, onConfirm, loading }) => {
  const [receivedItems, setReceivedItems] = useState(
    order.items.map(item => ({
      item: item.item._id, // Keep the item's _id to identify it in the backend
  ...
  const handleSubmit = () => {
    // This calls the onConfirm function provided by the parent,
    // which will then handle the inventory status update.
