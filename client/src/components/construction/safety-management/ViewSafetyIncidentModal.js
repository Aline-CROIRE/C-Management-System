// client/src/components/construction/safety-management/ViewSafetyIncidentModal.js
"use client";

import React from "react";
import ReactDOM from 'react-dom';
import styled from "styled-components";
import { FaTimes, FaHardHat, FaInfoCircle, FaCalendarAlt, FaUserTie, FaClipboardList, FaExclamationTriangle, FaDownload, FaPaperclip } from "react-icons/fa";
...
const ViewSafetyIncidentModal = ({ safetyIncident, onClose }) => {
  if (!safetyIncident) return null;
  ...
  return ReactDOM.createPortal(
    <ModalOverlay onClick={onClose}>
      <ModalContent onClick={(e) => e.stopPropagation()}>
        <ModalHeader>
          <ModalTitle>{safetyIncident.title || "Safety Incident Details"}</ModalTitle>
          <CloseButton type="button" onClick={onClose}><FaTimes /></CloseButton>
        </ModalHeader>
        <ModalBody>
          <SectionTitle><FaHardHat /> Incident Information</SectionTitle>
          <DetailGrid>
            <DetailItem><DetailLabel>Incident Date</DetailLabel><DetailValue>{formatDate(safetyIncident.incidentDate)}</DetailValue></DetailItem>
            <DetailItem><DetailLabel>Reported By</DetailLabel><DetailValue>{safetyIncident.reportedBy?.fullName || safetyIncident.reportedBy || 'N/A'}</DetailValue></DetailItem>
            <DetailItem><DetailLabel>Severity</DetailLabel><DetailValue><StatusBadge status={safetyIncident.severity}>{safetyIncident.severity}</StatusBadge></DetailValue></DetailItem>
          </DetailGrid>
          {safetyIncident.description && (
            <DetailItem>
              <DetailLabel><FaInfoCircle /> Description</DetailLabel>
              <DetailValue style={{ whiteSpace: 'pre-wrap' }}>{safetyIncident.description}</DetailValue>
            </DetailItem>
          )}
          {safetyIncident.actionsTaken && (
            <DetailItem>
              <DetailLabel><FaClipboardList /> Actions Taken</DetailLabel>
              <DetailValue style={{ whiteSpace: 'pre-wrap' }}>{safetyIncident.actionsTaken}</DetailValue>
            </DetailItem>
          )}
          {safetyIncident.notes && (
            <DetailItem>
              <DetailLabel><FaClipboardList /> Notes</DetailLabel>
              <DetailValue style={{ whiteSpace: 'pre-wrap' }}>{safetyIncident.notes}</DetailValue>
            </DetailItem>
          )}

          {safetyIncident.documents && safetyIncident.documents.length > 0 && (
            <>
              <SectionTitle><FaPaperclip /> Documents</SectionTitle>
              <ListContainer>
                {safetyIncident.documents.map((doc) => (
                  <li key={doc._id}>
...
export default ViewSafetyIncidentModal;